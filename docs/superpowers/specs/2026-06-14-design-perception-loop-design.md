...
`GEMINI_DESIGN_MODEL` should route design reasoning and structured handoff generation. The intended default is `gemini-3.1-pro-preview` when available because it is suited to multimodal reasoning, structured output, software-engineering plans, and agentic workflows. It is not an image generation model.
...
If a configured design model returns a model availability error such as 403 or 404, the command should fail closed for mutating workflows and print the fallback recommendation. It should not silently switch to a weaker model for handoff generation unless the user requested `--allow-fallback`. A fallback design model must satisfy the same capability contract as the primary route: multimodal input when screenshots are present and schema-valid structured JSON output for brief, perception, and handoff commands.
...
The handoff is the contract between Gemini and Codex. It should be specific enough to reduce repeated media inspection during implementation, while still allowing Codex to re-open screenshots or design assets when the task is visually ambiguous:
...
Gemini may propose code snippets inside the handoff, but Codex remains responsible for editing files, running checks, and deciding whether the code fits the repository.

### 5. Run The Codex Implementation Loop
...
- Missing, partially installed, or launch-failing browser automation returns a manual screenshot fallback warning instead of crashing the command.
...
- If pricing config is missing, corrupted, or lacks a configured model, budget-gated commands fail closed unless the user passes `--allow-unknown-cost`.
...
- Path traversal segments such as `..`, absolute paths inside run ids, and path separators inside run ids are rejected.
...
- WEBP dimensions should be read with a focused RIFF/VP8/VP8L/VP8X header parser in the MVP, without decoding full image pixels.
...
- Cost estimates are conservative soft gates, not billing guarantees. Pricing and model token accounting must be read from a configurable table so operators can update costs without source changes.
- Missing, corrupted, or incomplete pricing config fails closed for budget-gated commands unless `--allow-unknown-cost` is present.
...
- run-id generation, validation, path containment, and concurrent run directory creation are deterministic under test.
...
- coordinate normalization tests cover pixel and normalized provider outputs for multiple image sizes.
...
4. Implement `design generate` against the Gemini image API.
5. Implement bounded `design loop` using manual screenshot paths first.
6. Add optional browser screenshot automation later.
7. Add `provider=vision-banana` only when a real endpoint or model id is configured.
