...
function deliveryRecommendedAction(status) {
  ...
  if (status === "quarantined_events_present") {
    return "Run telemetry quarantine inspect --json, then telemetry quarantine retry --reason <reason> --dry-run before --write for receiver-policy fixes, or telemetry quarantine archive --reason <reason> --dry-run before --write for resolved cases.";
  }
