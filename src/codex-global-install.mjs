...
Context-pack reuse: when a relevant context pack already exists, pass it to \`plan-critique\`, \`patch-precheck\`, \`diff-review\`, or \`research-brief\` instead of resending the same raw project slice. Prefer \`--auto-context-pack\` for the project root artifact \`.gemini-agent/context/latest.json\`, or \`--context-pack <path>\` for a specific pack. If the pack is missing, stale, or unrelated to the current task, regenerate the context pack first. Add only narrow current input on stdin or \`--diff\` when the gate needs fresh details beyond the pack, and keep combined input bounded.
