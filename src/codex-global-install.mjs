...
Context-pack reuse: when a relevant context pack already exists, pass it to \`plan-critique\`, \`patch-precheck\`, \`diff-review\`, or \`research-brief\` instead of resending the same raw project slice. Prefer \`--auto-context-pack\` for the project root artifact \`.gemini-agent/context/latest.json\`, or \`--context-pack <path>\` for a specific pack. For current branch review, prefer \`gemini-agent diff-review --smart-diff\` so Gemini gets the reusable project context plus the fresh git diff; \`gemini-agent diff-review --auto-context-pack --diff\` remains the explicit equivalent. If the pack is missing, stale, or unrelated to the current task, run \`gemini-agent context-pack --bootstrap --write-artifact\` from the project root to regenerate it from the bounded root-file allowlist and current git diff. Add only narrow current input on stdin or \`--diff\` when the gate needs fresh details beyond the pack, and keep combined input bounded.
