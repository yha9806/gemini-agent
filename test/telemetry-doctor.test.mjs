...
test("runTelemetryDoctor points quarantined queues at quarantine inspect", async () => {
  ...
  const expected = "Run telemetry quarantine inspect --json, then telemetry quarantine retry --reason <reason> --dry-run before --write for receiver-policy fixes, or telemetry quarantine archive --reason <reason> --dry-run before --write for resolved cases.";
