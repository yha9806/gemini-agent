...
test("collectVisualGateSmoke blocks absolute screenshot paths before Gemini", async () => {
  const dir = await mkdtemp(join(tmpdir(), "visual-gate-smoke-"));
  const outside = await mkdtemp(join(tmpdir(), "visual-gate-outside-"));
  await writeFile(join(outside, "after.png"), minimalPng);

  const smoke = await collectVisualGateSmoke({
    cwd: dir,
    screenshots: [{ role: "actual", path: join(outside, "after.png") }],
  });

  assert.equal(smoke.status, "block");
  assert.deepEqual(smoke.media_summary, []);
  assert.ok(smoke.checks.some((check) => check.name === "file_readable" && check.status === "block"));
  assert.doesNotMatch(JSON.stringify(smoke), /after\.png|visual-gate-outside|\/tmp/);
});
