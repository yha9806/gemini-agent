...
test("README documents telemetry summary and bounded scheduler examples", async () => {
  ...
  assert.match(readme, /Telemetry priorities use artifact-review design scorecard coverage and average score/);
  ...
  assert.match(readme, /Latency priorities only use stage attribution when `pre_gemini_total` has at least 5 command-level samples/);
