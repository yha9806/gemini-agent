...
- `telemetry summary` reports aggregate usage, queue health, project/workspace attribution, palette-split quality, multimodal MIME/kind/byte coverage, and backfill media-manifest source adoption; it does not print raw prompt, response text, event ids, batch ids, paths, or media file names.
- Telemetry summary and economics aggregate context-pack preflight warning counts and rates without exposing raw gate input.
